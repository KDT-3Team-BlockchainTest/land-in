...
function GalleryBanner({ collection }) {
  if (collection.collectionStatus === "completed") {
    return (
      <section
        className="nft-gallery-page__banner nft-gallery-page__banner--completed"
        style={{ "--gallery-accent": collection.accentColor }}
      >
        <div className="nft-gallery-page__banner-icon" aria-hidden="true">
          🏆
        </div>
        <div>
    ...
    return (
      <section className="nft-gallery-page__banner nft-gallery-page__banner--ended">
        ...
        <div>
          ...
          <p className="nft-gallery-page__banner-text">
            수집한 NFT와 기록은 계속 내 컬렉션에 보관돼요.
          </p>
        </div>
      </section>
    );
  ...
  return (
    <section
      className="nft-gallery-page__banner nft-gallery-page__banner--active"
      style={{ "--gallery-accent": collection.accentColor }}
    >
      <div className="nft-gallery-page__banner-icon" aria-hidden="true">
        🎁
...
export default function NftGalleryPage() {
  ...
  const collection = getTrackedCollectionById(eventId, joinedEventIds);

  if (!collection) {
    return <Navigate to="/collection" replace />;
  }

  const nftCount = collection.collectedNfts.length;
  const remainingCount = Math.max(collection.landmarkCount - nftCount, 0);
  const progressPercent =
    collection.landmarkCount > 0
  ...
  return (
    <div className="nft-gallery-page">
      <div
        className="nft-gallery-page__hero"
        style={{ "--gallery-accent": collection.accentColor }}
      >
        <PlaceImage
          className="nft-gallery-page__hero-image"
        ...
        <div className="nft-gallery-page__hero-content">
          <p className="nft-gallery-page__hero-region">
      ...
      <main className="nft-gallery-page__content">
        <section className="nft-gallery-page__stats">
          <article className="nft-gallery-page__stat-card">
          ...
          <article className="nft-gallery-page__stat-card nft-gallery-page__stat-card--muted">
        ...
        <section className="nft-gallery-page__progress-card">
          <div className="nft-gallery-page__progress-header">
            <h2>컬렉션 진행률</h2>
          ...
          <ProgressBar
            ...
            className="nft-gallery-page__progress-track"
            fillClassName="nft-gallery-page__progress-fill"
          />
          <div className="nft-gallery-page__progress-meta">
            <span>
              {collection.collected}/{collection.landmarkCount} 수집
            </span>
        ...
        <section className="nft-gallery-page__gallery-header">
          <div>
            <h2>내 NFT 컬렉션</h2>
            <p>수집한 NFT는 이 컬렉션 안에서 계속 보관돼요.</p>
          </div>
        ...
        {nftCount === 0 ? (
          <section className="nft-gallery-page__empty">
            <div className="nft-gallery-page__empty-icon" aria-hidden="true">
              ✦
            </div>
            <h2>아직 수집한 NFT가 없어요</h2>
            <p>현장에서 NFC를 태그하면 첫 번째 NFT가 발행돼요.</p>
            <Link to={`/event/${collection.id}`} className="nft-gallery-page__empty-link">
              루트 보러 가기
            </Link>
          </section>
        ) : (
          <section className="nft-gallery-page__grid" aria-label="NFT 갤러리">
            {collection.collectedNfts.map((nft, index) => (
              <NftGalleryTokenCard
                key={nft.id}
                nft={nft}
                index={index}
            ...
            {Array.from({ length: remainingCount }).map((_, index) => (
              <NftGalleryTokenCard
                key={`locked-${collection.id}-${index}`}
                locked
                index={collection.collectedNfts.length + index}
              />
            ))}
          </section>
        )}
        ...
        <div className="nft-gallery-page__actions">
          <GradientActionButton
            label="방문 루트 보기"
            onClick={() => navigate(`/event/${collection.id}`)}
          />
          <Link to="/collection" className="nft-gallery-page__secondary-link">
            전체 컬렉션으로
          </Link>
        </div>
      </main>
