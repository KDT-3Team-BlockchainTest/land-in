import "./Home.css";
import { useNavigate } from "react-router-dom";
import ActiveEventCard from "../../components/common/ActiveEventCard/ActiveEventCard";
import FeaturedEventCard from "../../components/common/FeaturedEventCard/FeaturedEventCard";
import MoreEventsCard from "../../components/common/MoreEventsCard/MoreEventsCard";
import ProgressBanner from "../../components/common/ProgressBanner/ProgressBanner";
import SectionHeader from "../../components/common/SectionHeader/SectionHeader";
import UpcomingEventCard from "../../components/common/UpcomingEventCard/UpcomingEventCard";
import { getCollectionStats } from "../../data/collections";
import { activeEvents, featuredEvent, upcomingEvents } from "../../data/events";
import useJoinedEventIds from "../../hooks/useJoinedEventIds";

const homeIntro = {
  greeting: "안녕하세요,",
  userName: "지현님",
  title: "다음 여행을 찾아보세요",
};

const sectionCopy = {
  featured: {
    title: "추천 이벤트",
    description: "탭하면 루트 & 리워드 상세 보기",
  },
  active: {
    title: "진행 중인 이벤트",
    description: "탭해서 참여 & 루트 확인",
    actionLabel: "전체 보기",
  },
  upcoming: {
    title: "오픈 예정 캠페인",
    description: "탭해서 루트 미리 보기 & 알림 신청",
    actionLabel: "알림 신청",
  },
};

export default function Home() {
  const navigate = useNavigate();
  const { joinedEventIds, joinEvent } = useJoinedEventIds();
  const { nftCount, ongoingCount } = getCollectionStats(joinedEventIds);
  const progressSummary = {
    title: "내 진행 현황",
    description: `NFT ${nftCount}개 · 진행 중 ${ongoingCount}개`,
  };
  const activeEventCards = activeEvents.map((event) => ({
    ...event,
    joined: joinedEventIds.includes(event.id),
  }));

  return (
    <div className="page-layout">
      <main className="home-page__content">
        <section className="home-page__intro" aria-label="인트로">
          <p className="home-page__greeting">
            {homeIntro.greeting} <span>{homeIntro.userName}</span> <span aria-hidden="true">👋</span>
          </p>
          <h1 className="page-title">{homeIntro.title}</h1>
        </section>

        <ProgressBanner
          title={progressSummary.title}
          description={progressSummary.description}
          onClick={() => navigate("/my-progress")}
        />

        <section className="home-page__section">
          <SectionHeader
            title={sectionCopy.featured.title}
            description={sectionCopy.featured.description}
          />
          <FeaturedEventCard event={featuredEvent} />
        </section>

        <section className="home-page__section">
          <SectionHeader
            title={sectionCopy.active.title}
            description={sectionCopy.active.description}
            actionLabel={sectionCopy.active.actionLabel}
          />

          <div className="home-page__active-scroller">
            <div className="home-page__active-track">
              {activeEventCards.map((event) => (
                <ActiveEventCard
                  key={event.id}
                  event={event}
                  onJoin={() => joinEvent(event.id)}
                />
              ))}
              <MoreEventsCard />
            </div>
          </div>
        </section>

        <section className="home-page__section">
          <SectionHeader
            title={sectionCopy.upcoming.title}
            description={sectionCopy.upcoming.description}
            actionLabel={sectionCopy.upcoming.actionLabel}
          />

          <div className="home-page__upcoming-list">
            {upcomingEvents.map((event) => (
              <UpcomingEventCard key={event.id} event={event} />
            ))}
          </div>
        </section>
      </main>
    </div>
  );
}
