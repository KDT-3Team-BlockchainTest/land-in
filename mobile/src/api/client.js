...
async function request(method, path, body, options = {}) {
  ...
  if (text.trim()) {
    ...
    catch { throw Object.assign(new Error(`Unexpected response (${res.status})`), { status: res.status }); }
  }
  if (!res.ok) throw Object.assign(new Error(payload?.message || 'Server error'), { status: res.status });
