...
export default function CollectionPage() {
  ...
  const stats = useMemo(() => getCollectionStats(joinedEventIds), [joinedEventIds]);
  const filteredCollections = useMemo(
    () => getFilteredCollections(activeFilter, joinedEventIds),
    [activeFilter, joinedEventIds],
  );
  const collectedNfts = useMemo(() => getCollectedNfts(joinedEventIds), [joinedEventIds]);
  const showNftList = activeFilter === "nft";
  const isEmpty = showNftList ? collectedNfts.length === 0 : filteredCollections.length === 0;
      ...
      <main className="page-layout__content">
        ...
        {isEmpty ? (
          <section className="collection-page__empty">
            <div className="collection-page__empty-icon" aria-hidden="true">
              ✦
            </div>
            ...
            <p className="collection-page__empty-description">
              {showNftList
                ? "현장에서 NFC를 태그하면 첫 번째 NFT가 발행됩니다."
                : "탐험을 시작해 새로운 컬렉션에 참여해보세요."}
            </p>
          </section>
        ) : showNftList ? (
          <section className="collection-page__nft-section" aria-label="NFT 목록">
            <p className="collection-page__nft-caption">
              총 {collectedNfts.length}개의 NFT · 탭하면 해당 컬렉션으로 이동
            </p>
          ...
          <section className="collection-page__collection-list" aria-label="컬렉션 목록">
            {filteredCollections.map((collection, index) => (
              <CollectionOverviewCard key={collection.id} collection={collection} index={index} />
            ))}
          </section>
        )}
      </main>
